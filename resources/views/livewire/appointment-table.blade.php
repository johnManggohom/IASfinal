                    ...
                    <table class="w-full border-separate border-spacing-y-3 ">
                                  ...
                                  <th class="p-2  cursor-pointer text-sm font-thin text-gray-500">
                                    <div class="flex items-center justify-around">
                                        ...
                                        <span class="cursor-pointer" wire:click="sortBy('name','dresser')">
                                            <i class="fas fa-long-arrow-up {{ $sortColumnName === 'dresser'  && $sortDirection === 'asc' ? '' : 'text-muted'}}"></i>
                                            <i class="fas fa-long-arrow-down  {{ $sortColumnName === 'dresser'  && $sortDirection === 'desc' ? '' : 'text-muted'}}"></i>
                                        </span>
                                    </div>
                                </th>
                                <th class="p-2  cursor-pointer text-sm font-thin text-gray-500">
                        ...
                        <tbody>
                         ...
                         @foreach ($appointments as  $appointment)
                                ...
                                <td class="p-3 ">{{  $appointment->dresser->name}}</td>
                                  <td class="p-3 ">{{  $appointment->service->name}}</td>
                                  <td class="p-3 ">{{  $appointment->appointment_price}}</td>
                                  <td class="p-3 ">{{ \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $appointment->start_time)->format('M d Y g:i A')}}</td>
                                  ...
                                  @if ($appointment->status == 'finished')
                                     ...
                                     @else
                                            ...
                                            <td class="p-3 border-r bg-amber-100 "> {{ $appointment->status}}</td>
                                            @elseif ($appointment->status == 'rejected')
                                                <td class="p-3 border-r bg-orange-100 "> {{ $appointment->status}}</td>
                                                @elseif ($appointment->status == 'accepted')
                                                <td class="p-3   border-r bg-yellow-100 "> {{ $appointment->status}}</td>
                                             @endif
                                             ...
                                             <form  method="POST" action="{{ route('admin.appointment.update', $appointment->id) }}" onsubmit="return confirm('are you sure')">
                                                    @csrf
                                                <button type="submit" class="bg-cyan-300 p-3 text-dark hover:shadow-lg text-xs font-thin mr-1">Accept</button>
                                                </form>
                                                <form  method="POST" action="{{ route('admin.appointment.reject.rejectAppointment', $appointment->id) }}" onsubmit="return confirm('are you sure')">
                                                    @csrf   
                                                <button type="submit" class="bg-orange-300 p-3 text-white hover:shadow-lg text-xs font-thin mr-1">Reject</button>
                                                </form>
                                                <form  method="POST" action="{{ route('admin.appointment.finished.finishedAppointment', $appointment->id) }}" onsubmit="return confirm('are you sure')">
                                                    @csrf
                                                <button type="submit" class="bg-yellow-300 p-3 text-dark hover:shadow-lg text-xs font-thin mr-1">Finish</button>
